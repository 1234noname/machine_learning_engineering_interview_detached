output "wif_provider_url" {
  description = "Full resource URL of the WIF provider. Set as GitHub Actions environment secret WIF_PROVIDER on the `prod` environment."
  value       = module.wif.wif_provider_url
}

output "github_deployer_email" {
  description = "Deployer SA email. Set as GitHub Actions environment secret WIF_SERVICE_ACCOUNT on the `prod` environment."
...
output "cluster_ca_certificate" {
  description = "Base64-encoded GKE cluster CA certificate. Passed to environments/prod/app/ for Kubernetes/Helm provider config."
