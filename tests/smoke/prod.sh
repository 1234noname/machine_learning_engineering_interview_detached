#!/usr/bin/env bash
# Smoke checks against the prod app URL.
# Usage: APP_URL=<url> bash tests/smoke/prod.sh
#
...
#   bash tests/smoke/prod.sh <url>
URL="${APP_URL:-${1:-}}"

if [ -z "$URL" ]; then
    echo "ERROR: app_url is required (set APP_URL env var or pass as \$1)." >&2
    echo "Usage: APP_URL=<url> bash tests/smoke/prod.sh" >&2
    exit 1
fi
...
# ---------------------------------------------------------------------------
# Placeholder guard.
# Exit 0 when the URL is a known placeholder so the deployment is not
